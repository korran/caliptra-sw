...
pub fn elf2rom(elf_bytes: &[u8]) -> io::Result<Vec<u8>> {
    ...
    let rom_info_sym = symbols.iter().find(|s| s.name == "ROM_INFO").ok_or_else(|| other_err("ROM_INFO symbol not defined".to_string()))?;
    ...
    let rom_info_dest = result.get_mut(rom_info_start..rom_info_start + size_of::<RomInfo>()).ok_or_else(|| other_err("No space in ROM for ROM_INFO"))?;
