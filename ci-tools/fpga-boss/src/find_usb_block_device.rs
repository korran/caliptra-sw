use std::{path::{Path, PathBuf}, os::unix::prelude::OsStrExt};
...
pub fn find_usb_block_device(usb_path: &UsbPortPath) -> anyhow::Result<PathBuf> {
    ...
    for entry in std::fs::read_dir(path)? {
        let entry = entry?;
        if entry.file_name().as_bytes().starts_with(iface_prefix.as_bytes()) {
            ...
            for entry2 in iface_dir_iter {
                ...
                for entry3 in iter3 {
                    ...
                    for entry4 in iter4 {
                        ...
                        return Ok(Path::new("/dev").join(block_dev_name.file_name()))
                    }
                }
            }
        }
