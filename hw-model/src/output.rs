...
use std::{cell::{RefCell, Cell}, rc::Rc, io::Write};
    ...
    pub fn push_uart_char(&self, ch: u8) {
        ...
        match ch {
            TESTCASE_PASSED => {
                ...
                self.0.log_writer.borrow_mut().write_all(b"* TESTCASE PASSED\n").unwrap();
                self.0.exit_status.set(Some(ExitStatus::Passed));
            }
            TESTCASE_FAILED => {
                // This is the same string as printed by the verilog test-bench
                self.0.log_writer.borrow_mut().write_all(b"* TESTCASE FAILED\n").unwrap();
            ...
            _ => {
                writeln!(self.0.log_writer.borrow_mut(), "Unknown generic load 0x{ch:02x}").unwrap();
            }
        }
    }
}
...
impl Output {
    ...
    fn process_new_data(&mut self) {
        ...
        if self.output.len() == 0 {
            std::mem::replace(&mut self.output, new_data);
        } else {
